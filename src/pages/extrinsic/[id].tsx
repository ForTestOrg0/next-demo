...
export const getServerSideProps: GetServerSideProps<{ host: string; data: GetExtrinsicProps, tab: string, extrinsicIndex: string } & BareServerSideProps, { id: string }> = async (context) => {
  ...
  const data = await getExtrinsic(host, { events_limit: 1, extrinsic_index: extrinsicIndex });
  ...
  return (
    <PageContent>
      <Container className='flex-1'>
        <Text block bold className='mb-4 break-all'>Extrinsic#{extrinsicIndex}</Text>
        ...
        <Boundary className='mt-5'>
          <TabGroup>
            ...
            <TabPanels>
              <TabPanel><BlockEventsClient host={host} extrinsic_index={data.extrinsic_index} page={0} row={PAGE_ROW} order="asc" /></TabPanel>
            </TabPanels>
          </TabGroup>
        </Boundary>
      </Container>
