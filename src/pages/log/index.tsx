...
export const getServerSideProps: GetServerSideProps<
  { data: GetLogsProps; page: number } & BareServerSideProps
> = async (context) => {
...
export default function Page({
  ...
  const handleDownloadClick = () => {
    ...
    let tableName = `log-${data.logs[data.logs.length - 1].block_num}-${
      data.logs[0].block_num
    }.csv`;
      ...
      <Container className="flex-1">
        ...
        <Flex className="mt-5 flex justify-between">
          ...
          <Pagination
            ...
            pageSize={PAGE_ROW}
            current={page}
            urlRender={(_page) => `/log?page=${_page}`}
          />
        </Flex>
      </Container>
    </PageContent>
