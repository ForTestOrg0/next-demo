...
export const getServerSideProps: GetServerSideProps<{ host: string; data: GetBlockProps, tab: string, blockId: string } & BareServerSideProps, { id: string }> = async (context) => {
  ...
  const data = await getBlock(host, { only_head: true, ...(isNumber ? { block_num: parseInt(blockId) } : { block_hash: blockId }) });
  ...
  return (
    <PageContent>
      <Container className='flex-1'>
        <Text block bold className='mb-4 break-all'>Block#{blockId}</Text>
