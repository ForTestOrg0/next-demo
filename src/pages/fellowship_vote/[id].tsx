...
import { getFellowshipVotes, GetReferendaVotesProps } from '@/utils/api'
...
> = async (context) => {
  ...
  const data = await getFellowshipVotes(subdomain, {
    row: PAGE_ROW,
    page: page - 1,
    ...
    <PageContent>
      <Container className="flex-1">
        <Text block bold className="mb-4 break-all">
          For Fellowhsip Referenda #{referendumIndex}
        </Text>
        <Boundary>
        ...
        <Flex className="mt-5 flex-row-reverse">
          <Pagination
            ...
            pageSize={PAGE_ROW}
            current={page}
            urlRender={(_page) => `/fellowship_vote/${referendumIndex}?page=${_page}`}
          />
        </Flex>
      </Container>
    </PageContent>
