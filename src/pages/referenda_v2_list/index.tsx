...
import { getReferendaReferendums, GetReferendaReferendumsProps } from '@/utils/api'
...
> = async (context) => {
  ...
  const referendums = await getReferendaReferendums(subdomain, {
    row: PAGE_ROW,
    page: page - 1,
    origin,
    status: status,
  ...
  return (
    <PageContent>
      <Container className="flex-1">
        ...
        <Boundary>
          <ReferendaV2List referendums={referendums.list} />
          <Flex className="mt-5 flex-row-reverse">
            <Pagination
              total={referendums.count}
              pageSize={PAGE_ROW}
              current={page}
              urlRender={(_page) => `/referenda_v2_list?status=${status}&origin=${origin}&page=${_page}`}
            />
          </Flex>
        </Boundary>
      </Container>
