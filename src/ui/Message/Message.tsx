...
const createContainer = () => {
  ...
  container.className = 'fixed w-fit m-auto top-middle left-0 right-0 flex flex-col overflow-hidden z-40 p-4'
...
const createOne = (method: Method, content: JSX.Element | string) => {
  ...
  wrapper.className = 'flex items-center justify-center gap-middle py-2 px-4 mb-middle bg-white w-fit rounded-md animate-message-enter'
  ...
  contentEl.innerHTML = typeof content === 'string' ? content : renderToString(content)
...
const messageInstance = (method: Method, content: JSX.Element | string, duration?: number) => {
  if (!messageContainer) {
...
export const message = {
  info: (content: JSX.Element | string, duration?: number) => messageInstance('info', content, duration),
  warn: (content: JSX.Element | string, duration?: number) => messageInstance('warn', content, duration),
  error: (content: JSX.Element | string, duration?: number) => messageInstance('error', content, duration),
  success: (content: JSX.Element | string, duration?: number) => messageInstance('success', content, duration),
}
