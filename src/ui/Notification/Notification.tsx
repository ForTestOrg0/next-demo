...
const createOne = (config: Config, method: Method, onClose: () => void) => {
  ...
  wrapper.className = 'relative flex items-start justify-start gap-large p-large mb-middle bg-white w-96 rounded animate-notification-enter'
  content.className = 'flex flex-col items-start justify-start gap-small'
  closerIconWrapper.className = 'absolute top-2 right-2 w-7 h-7 p-1 rounded-full hover:cursor-pointer hover:bg-gray-800/10 transition duration-300'
  ...
  if (config.description) {
    ...
    description.innerHTML = typeof configContennt === 'string' ? configContennt : renderToString(configContennt)
