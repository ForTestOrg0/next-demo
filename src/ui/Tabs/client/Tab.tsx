...
export const Tab: React.FC<BareProps> = ({ children }) => {
  return (<HeadlessuiTab as={Fragment}>
    {({ selected }) => (
      <div
        className={clsx('py-2 text-sm focus:outline-none cursor-pointer text-sub-black font-semibold whitespace-nowrap', selected ? 'border-b-4 border-sub-network' : 'bg-sub-white text-black')}
      >
        {children}
      </div>
    )}
