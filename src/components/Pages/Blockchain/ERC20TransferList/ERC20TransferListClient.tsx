...
const Page: React.FC<Props> = ({ host, token, chain, ...props }) => {
  ...
  return (
    <div>
      <ERC20TransferList transfers={transfers.list} chain={chain} token={token} />
      {transfers?.count - props.row > 0 && (
        <ERC20TokenTransferListLink query={{ address: token?.contract?.toString() || '', symbol: token?.symbol?.toString() || '' }}>
